...
def normalize_cin(s):
    ...
    if not m:
        return {"type": "cin", "value": squash_spaces(s), "valid": False}
...
def normalize_date_ma(s):
    ...
    if not m:
        return {"type": "date", "value": squash_spaces(s), "valid": False}
    d, mo, y = map(int, m.groups())
    y = (y + 2000) if y < 100 and y < 50 else ((y + 1900) if y < 100 else y)
    ok = 1 <= d <= 31 and 1 <= mo <= 12 and 1900 <= y <= 2100
    return {
        "type": "date",
        "value": f"{y:04d}-{mo:02d}-{d:02d}" if ok else squash_spaces(s),
        "valid": ok,
    }
...
def normalize_phone_ma(s):
    ...
    return {
        "type": "phone",
        "value": f"+212{t}" if len(t) == 9 else squash_spaces(s),
        "valid": len(t) == 9,
    }
...
def normalize_receipt_no(s):
    ...
    if not m:
        return {"type": "receipt_no", "value": squash_spaces(s), "valid": False}
...
def normalize_name(s):
    return {"type": "name", "value": squash_spaces(s), "valid": bool(squash_spaces(s))}


def normalize_field(key: str, text: str) -> Dict[str, Any]:
    k = (key or "").lower()
    t = text or ""
    if any(x in k for x in ["cin", "cnie"]):
        return normalize_cin(t)
    if any(x in k for x in ["date", "deliv", "délivr", "naissance", "dob", "تاريخ"]):
        return normalize_date_ma(t)
    if any(x in k for x in ["tel", "tél", "phone", "gsm", "هاتف"]):
        return normalize_phone_ma(t)
    if any(x in k for x in ["recep", "récép", "receipt", "وصل", "رقم الوصل"]):
        return normalize_receipt_no(t)
    if "ice" in k:
        return normalize_ice(t)
    if re.search(r"\bif\b", k):
        return normalize_if(t)
    if any(
        x in k
        for x in ["commune", "arrondissement", "prefecture", "wilaya", "province"]
    ):
    ...
    return {
        "type": "text",
        "value": squash_spaces(ar2en_digits(t)),
        "valid": bool(squash_spaces(t)),
    }
