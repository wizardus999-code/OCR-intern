"""Template-based OCR extractor for Moroccan administrative documents."""
...
def _tx(x):
    """Get text from OCRResult or dict"""
    return (x.get("text") if isinstance(x, dict) else getattr(x, "text", "")) or ""


def _cf(x):
    """Get confidence from OCRResult or dict (float, -1 on failure)"""
    c = (
        x.get("confidence", x.get("conf"))
        if isinstance(x, dict)
        else getattr(x, "confidence", None)
    )
    try:
...
def _median(vals):
    """Calculate median of values, return None if empty"""
    if not vals:
        return None
    s = sorted(vals)
    return s[len(s) // 2]
...
def _build_tess_config(rel: Dict[str, Any]) -> str:
    cfg: List[str] = []
    if (psm := rel.get("psm")) is not None:
        cfg += ["--psm", str(psm)]
    if (oem := rel.get("oem")) is not None:
        cfg += ["--oem", str(oem)]
    if (dpi := rel.get("dpi")) is not None:
        cfg += ["-c", f"user_defined_dpi={int(dpi)}"]
    if rel.get("preserve_spaces"):
        cfg += ["-c", "preserve_interword_spaces=1"]
    if wl := rel.get("whitelist"):
        cfg += ["-c", f"tessedit_char_whitelist={wl}"]
    if bl := rel.get("blacklist"):
        cfg += ["-c", f"tessedit_char_blacklist={bl}"]
...
def _result_to_dict(r: Any) -> Dict[str, Any]:
    ...
    if hasattr(r, "__dict__") and hasattr(r, "text"):
        ...
        x, y, w, h = (
            bbox if isinstance(bbox, (list, tuple)) and len(bbox) == 4 else (0, 0, 1, 1)
        )
        return {
            "text": getattr(r, "text", ""),
...
class TemplateExtractor:
    ...
    def _abs_box(
        self, H: int, W: int, rel: Dict[str, float]
    ...
    def run(
        self, image: np.ndarray, template_key: str, engines: Dict[str, Any]
    ) -> Dict[str, Any]:
        if template_key not in self.templates:
            raise KeyError(
                f"Unknown template '{template_key}' in {self.templates_path}"
            )
        ...
        for section, fields in tpl["regions"].items():
            for name, rel in fields.items():
                ...
                if not lang_key:
                    is_ar = (section == "title" and name == "ar") or any(
                        "\u0600" <= ch <= "\u06FF" for ch in name
                    )
                ...
                # Initial pass to find best single result by area-weighted confidence
                ...
                # Collect token texts + confidences
                ...
                base_conf = statistics.median(token_confs) if token_confs else 0.0
                digit_conf = statistics.median(digit_confs) if digit_confs else None

                # Build candidates for TEXT SELECTION (score-only bias; does not affect reported conf)
                ...
                if digits_only:
                    score_candidates.append(
                        (
                            "digits",
                            digits_only,
                            (digit_conf if digit_conf is not None else base_conf) + 0.1,
                        )
                    )
                if best_text:
                    score_candidates.append(("token", best_text, base_conf))

                chosen_text = best_text or ""
                chosen_score = base_conf
                chosen_norm = {"type": "text", "value": "", "valid": False}

                for _, txt, sc in score_candidates:
                    nrm = normalize_field(f"{section}.{name}", txt)
                    # prefer valid, then higher score, then longer text
                    score = (1 if nrm.get("valid") else 0, sc, len(txt))
                    if score > (
                        1 if chosen_norm.get("valid") else 0,
                        chosen_score,
                        len(chosen_text),
                    ):
                        chosen_text, chosen_score, chosen_norm = txt, sc, nrm

                # Final values to store
                norm = chosen_norm
                best_text = chosen_text
                # Report *confidence*: special handling for receipt numbers
                field_key = f"{section}.{name}"
                is_receipt = name == "receipt_no" or field_key.endswith(".receipt_no")

                if is_receipt:
                    # For receipt numbers: boost confidence if format matches and we have digit confidence
                    if digit_conf is not None and re.fullmatch(
                        r"\d{4}/\d{3,5}", best_text
                    ):
                        best_conf = max(
                            float(digit_conf) * 2.0, 40.0
                        )  # Ensure at least 40% confidence
                    elif digit_conf is not None:
                ...
                # Now write the field ONCE
                out["fields"][field_key] = {
                    "value": best_text,
                    "norm": norm.get("value"),
