...
class PerformanceDashboard(QWidget):
    ...
    def _create_templates_tab(self) -> QWidget:
        ...
        self.template_table.setHorizontalHeaderLabels([
            "Template Type", "Count", "Avg. Time (s)", 
            "Avg. Confidence (%)", "Cache Hit Rate (%)"
        ])
    ...
    def _create_trends_tab(self) -> QWidget:
        ...
        self.metric_selector.addItems([
            "Processing Time", "Confidence Score", 
    ...
    def _update_templates_tab(self):
        ...
        for i, stat in enumerate(stats):
            ...
            self.template_table.setItem(i, 1, QTableWidgetItem(str(stat['document_count'])))
            self.template_table.setItem(i, 2, QTableWidgetItem(f"{stat['average_processing_time']:.2f}"))
            self.template_table.setItem(i, 3, QTableWidgetItem(f"{stat['average_confidence']:.1f}"))
            self.template_table.setItem(i, 4, QTableWidgetItem(f"{stat['cache_hit_rate']:.1f}"))
    ...
    def _update_errors_tab(self):
        ...
        for i, error in enumerate(errors):
            ...
            self.error_table.setItem(i, 2, QTableWidgetItem(str(error['total_documents'])))
            self.error_table.setItem(i, 3, QTableWidgetItem(f"{error['error_rate']:.1f}"))
