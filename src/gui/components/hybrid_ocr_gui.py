from PyQt6.QtWidgets import (QMainWindow, QWidget, QVBoxLayout, QHBoxLayout,
                            ...
                            QTextEdit, QComboBox, QGroupBox, QTableWidget,
                            QTableWidgetItem, QSplitter, QMenu, QTabWidget,
                            QCheckBox)
        ...
        with ThreadPoolExecutor(max_workers=self.batch_size) as executor:
            ...
            for future in as_completed(future_to_path):
                ...
                try:
                    ...
                    # Update status
                    status = "Processed from cache" if result.get('cached', False) else "Processed"
...
class MainWindow(QMainWindow):
    ...
    def _display_batch_results(self, results):
        ...
        for row, (path, data) in enumerate(results.items()):
            ...
            arabic_conf = f"{np.mean([r.confidence for r in arabic_results]):.1f}%" if arabic_results else "N/A"
            french_conf = f"{np.mean([r.confidence for r in french_results]):.1f}%" if french_results else "N/A"
            ...
            self.results_table.setItem(row, 3, QTableWidgetItem(f"{data['processing_time']:.2f}"))
    ...
    def show_document_results(self, item):
        ...
        q_img = QImage(rgb_image.data, new_width, new_height, 
